...
const getAvatarUrl = (user) => user?.profileImage?.thumbnail || user?.profileImage?.url || "";
...
const Messages = () => {
    ...
    const selectedUser = useMemo(
        () =>
            messagedUsers.find((item) => String(item._id) === String(selectedUserId)) ||
            (String(initialRecipient?._id) === String(selectedUserId) ? initialRecipient : null),
    ...
    const activeChatLabel = selectedUser?.name || (selectedUserId ? "New conversation" : "Select a chat");
    ...
    const handleSend = async (event) => {
        ...
        const result = await dispatch(
            sendMessage({ userId: normalizeId(selectedUserId), text: text.trim(), image })
        );
        ...
        if (result?.meta?.requestStatus === "fulfilled") {
            ...
            if (selectedUser) {
                dispatch(
                    upsertMessagedUser({
                        ...selectedUser,
                        latestMessageAt: result.payload?.createdAt || new Date().toISOString(),
                    })
                );
            }
            dispatch(getMessagedUsers());
        ...
        <div className="space-y-6">
            <div>
                ...
                <p className="text-sm text-[var(--muted)]">
                    Chat with restaurants, NGOs, and volunteers from one pickup-ready inbox.
                </p>
            </div>

            <div className="glass-panel grid min-h-[720px] overflow-hidden rounded-[36px] border border-white/70 lg:grid-cols-[360px_1fr]">
                <aside className="border-b border-white/70 bg-white/55 p-5 lg:border-b-0 lg:border-r">
                    <div className="flex items-center justify-between gap-3">
                        <div>
                            ...
                            <p className="text-xs text-[var(--muted)]">{messagedUsers.length} conversations</p>
                        </div>
                        <button
                            type="button"
                    ...
                    <div className="mt-5 max-h-[520px] space-y-2 overflow-auto pr-1">
                        ...
                        {!filteredUsers.length && (
                            <div className="rounded-[26px] bg-white/70 p-5 text-sm text-[var(--muted)]">
                                No chats found yet. Conversations will appear here after a message exists.
                            </div>
                        )}
                    </div>
                </aside>

                <section className="flex min-h-[720px] flex-col bg-[radial-gradient(circle_at_top_right,rgba(255,122,26,0.14),transparent_30%),linear-gradient(180deg,rgba(255,255,255,0.72),rgba(255,248,241,0.72))]">
                    <div className="flex items-center gap-3 border-b border-white/70 bg-white/65 px-5 py-4 backdrop-blur">
                        ...
                        <div className="min-w-0">
                            <h3 className="truncate font-display text-xl">{activeChatLabel}</h3>
                            <p className="truncate text-xs text-[var(--muted)]">
                                {selectedUserId ? selectedUserId : "Pick a conversation to begin"}
                            </p>
                        </div>
                    </div>
                        ...
                        {selectedUserId ? (
                            <div className="space-y-3">
                                {messages.map((message) => {
                                    const isMine = String(message.senderId) === String(userInfo?._id);
                                ...
                                {!messages.length && (
                                    <div className="mx-auto mt-20 max-w-sm rounded-[32px] bg-white/70 p-8 text-center">
                                        ...
                                        <h4 className="mt-4 font-display text-xl">No messages yet</h4>
                            ...
                            <div className="grid h-full place-items-center">
                                <div className="max-w-sm rounded-[36px] bg-white/70 p-8 text-center shadow-sm">
                                    ...
                                    <h4 className="mt-4 font-display text-2xl">Your pickup chats live here</h4>
                                    <p className="mt-2 text-sm leading-6 text-[var(--muted)]">
                                        Select a conversation from the inbox to continue coordinating.
                                    </p>
                                </div>
                            </div>
                        )}
                    ...
                    <form onSubmit={handleSend} className="border-t border-white/70 bg-white/70 p-4 backdrop-blur">
                        {image ? (
