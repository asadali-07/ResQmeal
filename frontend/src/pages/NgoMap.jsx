...
import { cancelClaim, createClaim, getNgoClaimedFoods } from "../store/claimSlice";
import { getAvailableFood } from "../store/foodSlice";
import { createNgo, getUserNgo, updateNgo } from "../store/ngoSlice";

const NgoMap = () => {
    const dispatch = useDispatch();
    const { availableFoods, loading: foodLoading, error: foodError } = useSelector(
        (state) => state.foodReducer
    );
    const { ngo, loading: ngoLoading, error: ngoError } = useSelector(
        (state) => state.ngoReducer
    );
    const { claim, volunteers, loading: claimLoading, error: claimError } = useSelector(
    ...
    const handleRefresh = async () => {
        ...
        await dispatch(
            getAvailableFood({ latitude: Number(coords.lat), longitude: Number(coords.lng) })
        );
    };
    ...
    const handleLocateUser = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            () => {
                setLocationMessage("Location access was denied. Using your saved NGO coordinates instead.");
            },
    ...
    const handleCancelClaim = async () => {
        ...
        if (result?.meta?.requestStatus === "fulfilled" && coords.lat && coords.lng) {
            dispatch(getNgoClaimedFoods());
            dispatch(
                getAvailableFood({
                    latitude: Number(coords.lat),
    ...
    const getRestaurantInfoState = (restaurant, returnTo = "/ngo", restaurantOwnerId = "") => ({
        restaurant,
        returnTo,
        restaurantOwnerId,
    });
    ...
    const deliveryNotices = notifications.filter(
        (item) => item.type === "CLAIM_ACCEPTED" || item.type === "DELIVERY_VERIFIED"
    );
                    ...
                    <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
                        ...
                        <div className="space-y-4">
                            <div className="glass-panel rounded-3xl border border-white/70 p-6">
                                <div className="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm text-[var(--muted)]">
                                            Change the search center manually or jump straight to your live location.
                                        </p>
                                    </div>
                                    <button
                                        type="button"
                                ...
                                {foodError && <p className="mt-3 text-sm text-red-500">{foodError}</p>}
                            </div>
                            ...
                            <div className="glass-panel rounded-3xl border border-white/70 p-6">
                                <div className="flex items-start justify-between gap-3">
                                    <div>
                                        ...
                                        <p className="mt-1 text-sm text-[var(--muted)]">
                                            The card updates from the map so you can review the pickup before claiming it.
                                        </p>
                                    </div>
                                    {selectedFood && (
                                            ...
                                            <div className="flex items-center justify-between gap-3">
                                                <div>
                                                    ...
                                                    <p className="mt-2 text-base font-semibold text-[var(--ink)]">
                                                        {selectedFood.restaurantId?.restaurantName || "Restaurant"}
                                                    </p>
                                                </div>
                                    ...
                                    <div className="mt-4 space-y-3 text-sm">
                                        <div className="rounded-2xl border border-white/80 bg-white/80 p-4">
                                            <p className="font-semibold text-[var(--ink)]">
                                                {claimedFood?.name || selectedFood?.name || "Claimed food"}
                                            </p>
                                        ...
                                        {getRestaurantId((claimedFood || selectedFood)?.restaurantId) ? (
                                            <Link
                                        ...
                                        {claim.status !== "delivered" && claim.status !== "cancelled" && (
                                            <button
                        ...
                        <div className="mt-5 grid gap-4 md:grid-cols-2">
                            {availableFoods.map((food) => (
                                ...
                                {!availableFoods.length && (
                                    <p className="text-sm text-[var(--muted)]">
                                    No nearby listings yet. Check that the restaurant and NGO addresses are within range, then refresh.
                                    </p>
                                )}
                        </div>
                    </div>
