...
const NgoRoutePreview = () => {
    const { foodId } = useParams();
    const dispatch = useDispatch();
    const mapRef = useRef(null);
    const token = import.meta.env.VITE_MAPBOX_TOKEN;
    const { availableFoods, food, loading: foodLoading, error: foodError } = useSelector(
        (state) => state.foodReducer
    );
    const { ngo, loading: ngoLoading, error: ngoError } = useSelector(
    ...
    const [locationNote, setLocationNote] = useState("Trying to read your live location.");
    ...
    const requestLiveLocation = () => {
        if (!navigator.geolocation) {
            setLocationNote("Live location is unavailable in this browser. Using your NGO profile coordinates.");
            return;
        }

        setLocationNote("Updating route from your live location.");
        navigator.geolocation.getCurrentPosition(
            (position) => {
                setLiveCoords([position.coords.longitude, position.coords.latitude]);
                setLocationNote("Using your live device location for this route preview.");
            },
            () => {
                setLocationNote("Live location permission was denied. Using your NGO profile coordinates.");
            },
    ...
    const routeFood = useMemo(() => {
        const listedFood = (availableFoods || []).find((item) => String(item._id) === String(foodId));
        if (listedFood) {
            return listedFood;
        }
    ...
    const ngoCoords = ngo?.location?.coordinates?.length === 2 ? ngo.location.coordinates : null;
    const originCoords = liveCoords || ngoCoords;
    const pickupCoords =
        routeFood?.location?.coordinates?.length === 2 ? routeFood.location.coordinates : null;
    ...
    useEffect(() => {
        const fetchRoute = async () => {
            ...
            try {
                ...
                setRouteData(null);
                setRouteError("Could not load a turn-by-turn route, so a direct line preview is shown instead.");
            } finally {
    ...
    useEffect(() => {
        const coordinates =
            routeFeature?.geometry?.coordinates || fallbackRoute?.geometry?.coordinates || [];
    ...
    const routeDistanceLabel = routeData?.distance
        ...
        : fallbackDistanceKm
          ...
          : "-";
    const routeDurationLabel = routeData?.duration ? formatDuration(routeData.duration) : "Direct preview";
    ...
    const mapMarkers = useMemo(() => {
        ...
        if (originCoords) {
            markers.push({
                ...
                label: "N",
                title: liveCoords ? "Your live location" : "Your NGO location",
                color: "var(--accent-2)",
            });
        }
    ...
    const addressLabel =
        routeFood?.restaurantId?.address?.formattedAddress || routeFood?.restaurantId?.address || "";
    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                    <h2 className="font-display text-3xl">Pickup route preview</h2>
                    <p className="text-sm text-[var(--muted)]">
                        Route planning from your NGO base or live device location to the selected pickup point.
                    </p>
                </div>
                <Link
                    to="/ngo"
            ...
            <div className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
                <div className="glass-panel overflow-hidden rounded-3xl border border-white/70">
                    <div className="border-b border-white/70 px-6 py-4">
                        <h3 className="font-display text-lg">{routeFood?.name || "Route map"}</h3>
                        <p className="text-sm text-[var(--muted)]">
                            Orange line shows the driving route when available. Blue line is the direct fallback preview.
                        </p>
                    </div>
                    <div style={{ height: 560 }}>
                        ...
                        >
                            ...
                            {routeFeature && (
                                <Source id="ngo-route" type="geojson" data={routeFeature}>
                                    <Layer {...routeLayer} />
                                </Source>
                            )}
                            {!routeFeature && fallbackRoute && (
                                <Source id="ngo-route-fallback" type="geojson" data={fallbackRoute}>
                            ...
                            <div className="mt-4 space-y-4">
                                ...
                                <div className="grid gap-3">
                                    <div className="flex items-center justify-between rounded-2xl border border-white/80 bg-white/80 px-4 py-3 text-sm">
                                        ...
                                        <span className="font-semibold text-right text-[var(--ink)]">
...
export default NgoRoutePreview;
