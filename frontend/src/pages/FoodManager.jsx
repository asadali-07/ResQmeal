...
const FoodManager = () => {
  ...
  const { food, foods, loading, error } = useSelector(
  ...
  const handleUpdateFood = async (event) => {
    ...
    const result = await dispatch(
      updateFood({ foodId: editingFood._id, ...payload }),
    );
    if (result?.meta?.requestStatus === "fulfilled") {
      handleCloseEdit();
      dispatch(getFoodListings());
  ...
  const liveMarkers = useMemo(() => {
    ...
    return Object.entries(liveLocations[trackingFoodId]).map(
      ([userId, location]) => ({
        id: `live-${userId}`,
  ...
  const getNgoInfoState = (ngo, returnTo = "/restaurant/food") => ({
    ngo,
    returnTo,
  });

  const getVolunteerInfoState = (volunteer, returnTo = "/restaurant/food") => ({
    volunteer,
    returnTo,
  });
  ...
  const canTrackFood = (foodId) => {
    ...
    return ["accepted", "picked_up"].includes(claimItem?.status);
  };
          ...
          {food ? (
            <div className="mt-4 space-y-3 text-sm text-(--muted)">
              ...
              <div className="flex items-center justify-between rounded-2xl border border-white/80 bg-white/80 px-4 py-3">
                <span>Status</span>
                <span
                  className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${getStatusClass(food.status)}`}
                >
                  {food.status}
                </span>
              </div>
          ...
          <div className="mt-6 space-y-3">
            {foods?.length ? (
              foods.map((listing) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <p className="font-semibold text-(--ink)">{listing.name}</p>
                    <span className="text-xs text-(--muted)">
                      <span
                        className={`rounded-full px-3 py-1 font-semibold uppercase ${getStatusClass(listing.status)}`}
                      >
                        {listing.status}
                      </span>
                    </span>
                    ...
                    {listing.status === "available" && (
                      <>
                        <button
                          ...
                          className="rounded-full border border-(--accent-2) px-3 py-1 text-xs font-semibold text-(--accent-2)"
                        >
                          Edit
                        </button>
                        <button
            ...
            {selectedFood ? (
              <div className="mt-4 space-y-3 text-sm">
                ...
                <div className="flex items-center justify-between rounded-2xl border border-white/80 bg-white/80 px-4 py-3">
                  <span>Status</span>
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-semibold uppercase ${getStatusClass(selectedFood.status)}`}
                  >
                    {selectedFood.status}
                  </span>
                </div>
                <div className="grid gap-2 sm:grid-cols-2">
                  ...
                  {canTrackFood(selectedFood._id) ? (
                    ...
                    <div className="rounded-2xl border border-dashed border-white/80 px-4 py-3 text-xs text-(--muted) sm:col-span-2">
                      Tracking becomes available once a volunteer accepts this
                      claim.
                    </div>
                  )}
                </div>
                ...
                >
                  ...
                  {notice.ngoId && (
                    <Link
                      ...
                      className="rounded-full w-21 border border-(--accent) px-4 py-2 text-xs font-semibold text-(--accent)"
                    >
                      NGO info
                    </Link>
                  ) }
                  {notice.foodId &&
                    ["CLAIM_ACCEPTED", "PICKUP_VERIFIED"].includes(
                      notice.type,
                    ) && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        <button
