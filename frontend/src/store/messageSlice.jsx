  ...
  async ({ userId, text, image }, { rejectWithValue }) => {
    try {
      ...
      const formData = image
        ? buildFormData({ text, imageUrl: image })
        : { text };
      const response = await api.post(
        `/messages/send/${recipientUserId}`,
        formData,
        {
...
export const updateMessage = createAsyncThunk(
  ...
  async ({ messageId, text }, { rejectWithValue }) => {
    try {
      const response = await api.patch(`/messages/update/${messageId}`, {
        text,
      });
      ...
      return rejectWithValue(
        getErrorMessage(error, "Failed to update message"),
      );
    }
  },
);
